...
export const casesData: CaseStudy[] = [
  ...
  },
  {
    ...
    fullDescription:
      "Global Olive Corporation — полнофункциональная e-commerce платформа для инвестирования в оливковые деревья. Пользователи могут покупать деревья, получать сертификаты владения, дарить деревья другим и отслеживать активы. Включает админ-панель для управления инвентарем, заказами и пользователями.",
    challenge:
      "Клиент хотел создать уникальную платформу для альтернативных инвестиций в реальные активы. Необходимо было реализовать сложную логику владения, передачи активов и генерации сертификатов.",
    solution:
      "Разработали платформу с системой NFT-сертификатов владения, интеграцией с платёжными системами и личным кабинетом для отслеживания инвестиций. Реализовали функцию дарения деревьев и автоматическую генерацию сертификатов.",
    ...
    results: [
      ...
      { label: "Деревьев продано", value: "1000+" },
      { label: "Сертификаты", value: "NFT" },
      { label: "Конверсия", value: "8%" },
    ],
    tags: ["Next.js", "Stripe", "PostgreSQL", "AWS", "NFT", "React"],
    testimonial: {
      text: "YappiX создали уникальную платформу, которая позволяет людям инвестировать в реальные активы. Продажи превзошли наши ожидания.",
      author: "Marco Rossi",
      role: "CEO Global Olive",
    },
    duration: "5 месяцев",
    team: "7 человек",
    year: "2024",
