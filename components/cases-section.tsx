...
const casesByLocale = {
  ...
  en: [
    { slug: "myunion-platform", title: "MyUnion Pro — Trade Union Management Platform", category: "SaaS / AI", description: "Full-featured web application for trade union management with AI chatbots and automated document generation.", metrics: ["1 person", "3 months", "50K+ users"], tags: ["Next.js", "GPT-4", "PostgreSQL", "LangChain"], image: myunionListImage, video: undefined },
...
function VideoCard({ src, title, poster, eager }: { src: string; title: string; poster?: string; eager?: boolean }) {
...
function LazyImage({ src, alt, eager }: { src: string; alt: string; eager?: boolean }) {
  ...
  // H2/прокси иногда оставляют img без onLoad/onError — убираем вечный скелет
  useEffect(() => {
    if (!isVisible) return
    const t = window.setTimeout(() => setIsLoaded(true), 12000)
